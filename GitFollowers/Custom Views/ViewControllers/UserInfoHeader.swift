...
class UserInfoHeader: UIViewController {
    
    let avatarImageView = GitFollowerAvatarImageView(frame: .zero)
    let usernameLabel = GitFollowersTitleLabel(textAlignment: .left, fontSize: 34)
    let nameLabel = SecondaryTitleLabel(fontSize: 18)
    let locationImageView = UIImageView()
    let locationLabel = SecondaryTitleLabel(fontSize: 18)
    let bioLabel = GitFollowersBodyLabel(textAlignment: .left)
